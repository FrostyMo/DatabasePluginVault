...
func (b *databaseBackend) connectionWriteHandler() framework.OperationFunc {
	return func(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
		...
		delete(data.Raw, "ci_name")
		delete(data.Raw, "emails")
		...
		// Replace in-memory instance if exists
		old := b.conn.Put(name, &dbPluginInstance{
			Database: engine,
			Engine:   engine,
...
func (b *databaseBackend) connectionDeleteHandler() framework.OperationFunc {
	return func(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
		...
		if err := b.conn.ClearConnection(name); err != nil {
			return nil, err
		}
		return nil, nil
	}
